...
select
    ...
    EMAIL_VERIFIED,
    CC_LAST_UPDATED,
    listagg(cast(service_type_id as varchar(3)), ', ')
        --within group(order by service_type_id)
        as service_type_ids,
    listagg(cast(service_status_id as varchar(3)), ', ')
        --within group(order by service_status_id)
        as service_status_ids,
    CS_LAST_UPDATED
from (
    select
